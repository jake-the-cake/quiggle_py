...
class SocketController:
	...
	def check_and_kill_port(self) -> None:
			...
			if conn.laddr.port == self.port and conn.status == 'LISTEN':
				try:
					...
					if 'http' in process.name().lower() or 'socket' in process.name().lower():
						...
						print_note(f"Terminated process '{process.name()}' using port {self.port}.")
						return
					else:
						raise RuntimeError(
		...
		print_note(f"Port {self.port} is not in use or could not be resolved.")
	...
	def start_socket(self) -> None:
		...
		while attempt < 2:  # Retry once after killing a potential conflict
			try:
				...
				print_note('Socket Server Starting...')
				return
			except socket.error as e:
				print_error('Socket Start', e)
				if attempt == 0:
	...
	def accept_connections(self) -> None | SocketAddressType:
		...
		try:
			...
			print_error('Socket Connection', e)
			return None
	...
	def close_socket(self) -> None:
		...
		if self.server_socket:
			...
			print_note(MESSAGES['closed'](self.__class__.__name__))